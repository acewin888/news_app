package com.example.android.newsapp.mvp.entity;

/**
 * Created by kevinsun on 11/22/17.
 */

public class Constant {

    public static final String API_KEY = "c10a1af09bbc4567844a7f5c7ffd289c";


    public static final String ABC_NEWS = "abc-news";
    public static final String ASSOCIATED_PRESS = "associated-press";
    public static final String BBC_NEWS = "bbc-news";
    public static final String BLLOMBERG = "bloomberg";


    public static final String BBC_SPORT = "bbc-sport";
    public static final String BUSINESS_INSIDER = "business-insider";
    public static final String BUZZFEED = "buzzfeed";
    public static final String CBS_NEWS = "cbs-news";
    public static final String CNN = "cnn";
    public static final String ESPN = "espn";
    public static final String FORTUNE = "fortune";
    public static final String FOX_NEWS = "fox-news";
    public static final String GOOGLE_NEWS = "google-news";
    public static final String NBC_NEWS = "nbc-news";


    public static final String SHAREPERFERENCE ="SHARE_PREFERENCE";
    public static final String NIGHT_THEME_MODE = "night_theme_mode";


    public static final String INIT_DB = "init_database";
}
